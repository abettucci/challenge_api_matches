...
def calculate_similarity(title1: str, title2: str) -> float:
    """Calcular similitud básica entre dos títulos"""
    # Normalizar títulos
...
def get_all_pairs():
    ...
    try:
        ...
        for pair in pairs:
            if 'similarity_score' in pair:
                pair['similarity_score'] = float(pair['similarity_score'])
...
def get_pair(pair_id):
    ...
    try:
        ...
        if 'Item' not in response:
            return create_response(404, {
                'status': 'error',
                'message': f'Par con ID {pair_id} no encontrado'
            })
